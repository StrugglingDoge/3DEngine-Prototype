...
{
    //TODO: Refactor, only want it to do what I need it to do.
    
    private GraphicsDevice graphicsDevice = null;
    private GameWindow gameWindow = null;

    private MouseState mState = default(MouseState);
    private MouseState cacheState = default(MouseState);
    ...
    /// </summary>
    public Basic3DCamera(GraphicsDevice gfxDevice, GameWindow window)
    {
    ...
    private void EditingUiControlsLayout(GameTime gameTime)
    {
        ...
        else if (kstate.IsKeyDown(Keys.LeftShift) == true)
        {
            MoveDown(gameTime);
        }
        ...
        {
            mouseLookIsUsed = !mouseLookIsUsed;
            Game1.Instance.IsMouseVisible = !mouseLookIsUsed;
        }

        if (mouseLookIsUsed)
        {
            // We don't want to update the position if it was just forcefully reset for exiting the safe area.
            if (!justReset)
            {
                Vector2 diff = state.Position.ToVector2() - mState.Position.ToVector2();
                if (diff.X != 0f)
                    RotateLeftOrRight(gameTime, diff.X);
                // Stop the camera from flipping out when looking too far down/up.
                if (diff.Y != 0f && diff.Y > 0 ? LookAtDirection.Y > -0.98f : LookAtDirection.Y < 0.98f)
                    RotateUpOrDown(gameTime, diff.Y);
            }
            else
            {
            ...
            //TODO: Finally working, make a global variable, no reason to do this every frame.
            /*
             *
             * Safebounds are used to create an imaginary rectangle around the center of the screen.
             * This 'safebound' is essentially what the mouse cursor is allowed to move within while
             * mouse input is being captured. When the cursor leaves this area, the position will be
             * reset to center of the screen.
             * 
             */
            var safeBoundX1 = (gameWindow.ClientBounds.Width / 2f) + 300f;
            var safeBoundY1 = (gameWindow.ClientBounds.Height / 2f) + 300f;
            var safeBoundX2 = (gameWindow.ClientBounds.Width / 2f) - 300f;
            var safeBoundY2 = (gameWindow.ClientBounds.Height / 2f) - 300f;
            
            // Essentially creates a 'safe' area that the mouse is allowed to move within, if it moves out the position is reset.
            if (state.Position.X > safeBoundX1 || state.Position.X < safeBoundX2 || state.Position.Y > safeBoundY1 || state.Position.Y < safeBoundY2)
            {
                Mouse.SetPosition(gameWindow.ClientBounds.Width / 2, gameWindow.ClientBounds.Height / 2);
                // Don't let the game think the user was the one that did this!
                justReset = true;
            }
